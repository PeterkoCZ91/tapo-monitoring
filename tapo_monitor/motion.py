...
2. hold — auto-track has the lens on a subject (``track_hold``, only while tracking);
3. pan-limit guard — overrides the hold once the lens has been out of bounds for
   ``hold_grace`` seconds, so a dwell never licenses staring into a wall for long;
4. scheduled preset recall.
...
def decide(requester, *, privacy_on=False, hold=False, autotrack_on=False,
           out_of_bounds_for=0.0, hold_grace=0.0) -> Decision:
